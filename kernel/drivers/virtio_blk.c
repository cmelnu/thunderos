...
static int virtqueue_init(virtio_blk_device_t *dev, uint32_t queue_size)
{
    ...
    if (!desc_region) {
        hal_uart_puts("virtio_blk: Failed to allocate descriptor ring\n");
        return -1;
    }
    ...
    if (!avail_region) {
        hal_uart_puts("virtio_blk: Failed to allocate available ring\n");
    ...
    if (!used_region) {
        hal_uart_puts("virtio_blk: Failed to allocate used ring\n");
        dma_free(desc_region);
...
{
    hal_uart_puts("virtio_blk: Initializing VirtIO block device\n");
    
    /* Allocate device structure */
    g_blk_device = (virtio_blk_device_t *)kmalloc(sizeof(virtio_blk_device_t));
    if (!g_blk_device) {
        hal_uart_puts("virtio_blk: Failed to allocate device structure\n");
        return -1;
    }
    ...
    if (magic != VIRTIO_MAGIC) {
        hal_uart_puts("virtio_blk: Invalid magic value\n");
        kfree(g_blk_device);
        g_blk_device = NULL;
        return -1;
    }
    ...
    if (!(status & VIRTIO_STATUS_FEATURES_OK)) {
        hal_uart_puts("virtio_blk: Features not accepted by device\n");
        kfree(g_blk_device);
        g_blk_device = NULL;
        return -1;
    }
